...
from densenet import DensnetYolo
import argparse
...
def main():
	...
	model = DensnetYolo(cfgfile, use_cuda=use_cuda)
...
def savemodel(epoch, nsamples, curmax=False):
	...
	if curmax:
		torch.save(cur_model,'%s/localmax.weights' % (backupdir))    #cur_model.save_weights('%s/localmax.weights' % (backupdir))
	else:
		torch.save(cur_model,'%s/%06d.weights' % (backupdir, epoch))
...
if __name__ == '__main__':
	...
	parser.add_argument('--config', '-c',
						type=str, default='cfg/densenet.cfg', help='network configuration file')
